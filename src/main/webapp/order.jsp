...
<div class="head ding">
    <div class="wrapper clearfix">
        <div class="clearfix" id="top"><h1 class="fl"><a href="indexServlet"><img src="img/logo.png"/></a></h1>
            <div class="fr clearfix" id="top1"><p class="fl"><a href="#" id="login">登录</a><a href="#"
                                                                                               id="reg">注册</a>
            </p>
                <form action="searchProductServlet" method="post" class="fl"><input required type="text" name="key"
                                                                                    placeholder="热门搜索：干花"/><input
...
<div class="order cart mt"><!-----------------site------------------->
    ...
    <div class="orderCon wrapper clearfix">
        ...
        <div class="orderR fr">
            <div class="msg">
                <h3>订单内容<a href="toCartServlet" class="fr">返回购物车</a></h3>

                <c:forEach items="${cartList}" var="cart" varStatus="status">
                    <ul class="clearfix">
                        <li class="fl">
                            <img width="100" height="100"
                                 src="${pageContext.request.contextPath}/upload/product/${productList[status.index].product_photo}">
                        </li>
                        <li class="fl">
                            <p>${productList[status.index].product_name}</p>
                            <p>单价：￥${cart.product_price}</p>
                            <p>数量：${cart.product_quantity}</p>
                        </li>
                        <li class="fr">￥${cart.product_price * cart.product_quantity}.00</li>
                    </ul>
                </c:forEach>
            </div>

            <div class="tips">
                <p><span class="fl">商品金额：</span><span class="fr">￥${totalPrice}.00</span></p>
            ...
            <!--<input type="button" name="" value="去支付">--> <a href="javascript:void(0);"
                                                                   onclick="js_method('${order.order_id}')" ;
                                                                   class="pay">去支付</a></div>
    </div>
</div><!--编辑弹框--><!--遮罩-->
<div class="mask"></div>
...
<div class="footer">
    ...
    <p class="dibu">家居电子商城&copy;2000-2020公司版权所有 京ICP备000001-11备0000111000号<br/>
        违法和不良信息举报电话：888-888-8888，本网站所列数据，除特殊说明，所有数据均出自我司实验室测��</p></div>
<script>
    $(function () {
        // js_method实现
        window.js_method = function (orderId) {
            console.log("Processing order:", orderId); // 调试用

            var addressId = $(".addres .addre.on").find("#addressId").val();
            console.log("Selected address:", addressId); // 调试用
            location.href = 'toPayServlet?oid=' + orderId + '&aid=' + addressId;
        }
    });
</script>
